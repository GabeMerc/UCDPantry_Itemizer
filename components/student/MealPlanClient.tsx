  ...
  useEffect(() => {
    ...
    for (const day of WEEKDAYS) {
      ...
      for (const mt of mealTypes) {
        ...
        if (pool.length > 0) {
          // Try to place a recipe, checking budget
          let placed = false;
          ...
          if (!placed) {
            // Couldn't fit within budget — try unknown type as last resort
          ...
          if (unknowns.length > 0) {
            ...
            plan[day][mt] = { ...unknowns[idx], meal_type: "unknown" };
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            Mon–Fri · {activeMealTypes.map((t) => MEAL_TYPE_LABELS[t]).join(", ")}
          </p>
        </div>
  ...
  if (!recipe) {
    return (
      <div className="bg-gray-100 rounded-lg p-2 flex flex-col items-center justify-center min-h-[100px] text-xs text-gray-400 text-center">
        ...
        <a
          href="/swipe"
          className="text-ucd-blue hover:underline mt-1"
        >
          Swipe more
        </a>
      </div>
